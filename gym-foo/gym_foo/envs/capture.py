...
class CaptureRules:
  ...
  def getProgress(self, game):
    blue = 1.0 - (game.state.getBlueFood().count() / (self._initBlueFood))
    red = 1.0 - (game.state.getRedFood().count() / (self._initRedFood))
    moves = len(self.moveHistory) / (game.length)
...
def runGames( layouts, agents, display, length, numGames, record, numTraining, redTeamName, blueTeamName, muteAgents=False, catchExceptions=False ):
  ...
  if numGames > 1:
    ...
    redWinRate = [s > 0 for s in scores].count(True)/ (len(scores))
    blueWinRate = [s < 0 for s in scores].count(True)/ (len(scores))
